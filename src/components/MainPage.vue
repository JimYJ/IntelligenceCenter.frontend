<template>
  <el-container class="main-container">
    <el-aside width="210px" class="main-menu">
        <el-menu :router="true">
          <img alt="logo" width="200px" height="46px" src="../assets/logo.png">
          <el-menu-item index="/task/list"><el-icon><List /></el-icon>任务中心</el-menu-item>
          <el-menu-item index="/info/list"><el-icon><Document /></el-icon>信息中心</el-menu-item>
          <el-menu-item index="/option"><el-icon><Setting /></el-icon>选项</el-menu-item>
        </el-menu>
    </el-aside>
    <InfoDetail v-if="$route.path==='/info/detail'"/>
    <InfoList v-if="$route.path==='/info/list'"/>
    <TaskList v-if="$route.path==='/task/list'"/>
    <OptionPage v-if="$route.path==='/option'"/>
  </el-container>
</template>

<script setup>
import InfoDetail from './InfoDetail.vue'
import InfoList from './InfoList.vue'
import TaskList from './TaskList.vue'
import OptionPage from './OptionPage.vue'
import { List, Document, Setting } from '@element-plus/icons-vue'
</script>

<style scoped>
.main-container .el-header {
  /* position: relative; */
  background-color: var(--el-color-primary-light-7);
  color: var(--el-text-color-primary);
}
.main-container .el-aside {
  color: var(--el-text-color-primary);
  background: var(--el-color-primary-light-8);
}
.main-container .el-menu {
  border-right: none;
}
.main-container .el-main {
  padding: 0;
}
.main-container .toolbar {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  height: 100%;
  right: 20px;
}
.main-container .main-menu  {
  border-right: 1px solid #dcdfe6; /* 设置右边的边框 */
}
.main-container .info-detail, .list-page{
  margin-left: 20px;
  margin-top: 17px;
}
.main-menu{
  height: 97vh;
  background-color: #fff !important;
}
.pagination-block + .pagination-block {
  margin-top: 50px;
}
.pagination-block{
  margin-top: 15px;
  margin-left: 80px;
}
.pagination-block .example-demonstration {
  margin-bottom: 32px;
}
</style>